...
- **경로 A — 배치 캐시:** 워크넷 공공 API와 사람인 공식 OAPI로만 4개 직군 스킬맵을 주기적으로(길게는 분기, 짧게는 주간) 갱신해 서버에 정적 데이터로 동봉합니다. 런타임 툴은 이 캐시 위에서 추론만 하므로 빠르고 안정적입니다.
...
### 경로 A — 배치 캐시 (백그라운드, API 전용)

```text
워크넷 공공 API + 사람인 공식 OAPI
↓
직무 키워드별 검색 (company 필터로 대기업 포함)
↓
중복 제거 / 정규화
↓
...
- SKT·삼성SDS·현대모비스 같은 대기업 공고도 상당수가 사람인/워크넷에 동시 게시되므로 `company` 필터로 커버합니다.
