...
| --- | --- | --- | --- |
| 직업정보 API(고용24, 212) | ✅ | 직업별 담당업무·능력·지식·자격·임금/전망(정량 점수) | **요구역량 1차 백본** |
| 직무정보 API(고용24, 215L01/NCS) | ✅ | 수행직무 텍스트→NCS 능력단위 | 보조(퍼지, 품질 편차) |
...
- 경로 A 스킬맵 = 직업정보 212 **직업 앵커 역량(정량 점수)**에 사람인·공채 **시장수요 메타**를 얹어 산출. 개별 공고 본문에 의존하지 않으므로 개인 자격으로 구축 가능합니다.
...
**경로 A 백본 = 직업정보 212(직업 앵커링).** 품질 실측 결과 215L01 자유텍스트는 직군 편차가 커(백엔드 0건, 프론트/AI 노이즈 범벅) 보조로 강등하고, 212 직업정보를 1차 백본으로 채택.

| 서비스 | 코드 | 개인 | 핵심 파라미터 | 역할 |
| --- | --- | --- | --- | --- |
| 직업정보 검색 | 212L01 | ✅ | target=JOBCD, srchType=K, keyword | 직군→jobCd 앵커 |
| 직업정보 상세(7종) | 212D01~D07 | ✅ | target=JOBDTL, jobGb=1, jobCd, dtlGb=1~7 | 담당업무/능력·지식/자격/임금·전망 |
| 직무정보 표준직무기술서 | 215L01 | ✅ | jobCont(텍스트), limit, returnType=JSON | 보조(퍼지, 품질 편차) |
| 채용정보 목록/상세 | 210L01 / 210D01 | ❌ | — | 개인 불가 → 미사용 |
| 공채속보 목록/상세 | (코드 미확인) | ✅ | — | 대기업 수요 신호(예정) |

**212 상세 7종(dtlGb):** 1 요약(`jobSum`/`way`) · 2 하는일(`execJob`=담당업무 실텍스트) · 3 교육자격훈련(`certNm`/학력분포) · 4 임금만족도전망(`sal`/`jobSatis`/`jobProspect`) · 5 능력지식환경(`jobAbil`/`Knwldg`+중요도 점수) · 6 성격흥미가치관 · 7 업무활동.

**직군 → 직업(jobCd) 앵커 매핑(실측):**

| 직군 | jobCd | 직업명 | 검색어 |
| --- | --- | --- | --- |
| Backend | K000001176, K000000853 | 응용소프트웨어개발자, 시스템소프트웨어개발자 | 소프트웨어 |
| Frontend | K000001106 | 웹개발자(웹 프로그래머) | 웹 |
| AI/Data | K000001080, K000001134 | 데이터분석가(빅데이터), 데이터시스템전문가 | 빅데이터/데이터 |
| Security | K000000832 | 정보 보안 전문가 | 보안 |

> ⚠️ 검색어 어휘 주의: `인공지능`/`정보보안`/`머신러닝`은 0건. `보안`·`빅데이터`·`데이터`로 검색해야 잡힘. KNOW 직업사전에 독립 "AI개발자" 직업이 없어 AI는 데이터 직군을 프록시로 쓰고, 최신 AI 요구는 경로 B로 보완.
> 215L01은 XML이 500 → `returnType=JSON` 필수.
